...
class MetadataManager {
	...
	getDataset = async (
		...
		const logger = new Logger();
		const systemDestinationURL = await appGlobalConfig[activeSystem]
			.dataToURL;
		const dataSet = await appGlobalConfig[activeSystem][activeBatch][
			activeJob
		].dataSet;
		const auth = authenticator.getAPIAuth(APIAuthConfig);

		const payloadURL = `${
			_.endsWith(systemDestinationURL, '/')
				? systemDestinationURL
				: `${systemDestinationURL}/`
		}api/identifiableObjects/${dataSet.id}.json`;
		...
		if (response.data) {
			...
			if (_.has(axiosResponse.data, 'message')) {
				logger.printLogMessageInConsole(
					...
					`${chalk.green(
						chalk.bold('Message: ')
					)} - ${chalk.bold(axiosResponse.data.message)}`,
					activeSystem
				);
			} else {
				logger.printLogMessageInConsole(
					'success',
					`Dataset <${chalk.bold(
						chalk.green(axiosResponse.data.name)
					)}> with id <${chalk.bold(
						chalk.green(axiosResponse.data.id)
					)}> is successfully created in system`,
					activeSystem
				);
			}
		}
	...
	getDataDetail = async (
		...
		const authenticator = new Authenticate();
		const logger = new Logger();
		const systemSourceURL = await appGlobalConfig[activeSystem]
			.dataFromURL;
		const dxs = await appGlobalConfig[activeSystem][activeBatch][
			activeJob
		].dx;
		const auth = authenticator.getAPIAuth(APIAuthConfig);
		...
		for (const dx of await dxs.data) {
			const payloadURL = `${
				_.endsWith(systemSourceURL, '/')
					? systemSourceURL
					: `${systemSourceURL}/`
			}api/identifiableObjects/${dx.id}.json`;
			const response = await axios.get(
				payloadURL,
	...
	sendSystem = async (payload, activeSystem) => {
		const authenticator = new Authenticate();
		const logger = new Logger();
		const auth = authenticator.getAPIAuth(APIAuthConfig);
		...
		if (_.has(APIAuthConfig, 'url')) {
			if (_.endsWith(APIAuthConfig.url, '/')) {
				url = `${APIAuthConfig.url}api/systems`;
			} else {
				url = `${APIAuthConfig.url}/api/systems`;
			}
		}
		...
		if (_.has(axiosResponse.data, 'message')) {
			logger.printLogMessageInConsole(
				'success',
				`${chalk.green(chalk.bold('Message: '))} - ${chalk.bold(
					axiosResponse.data.message
				)}`,
				activeSystem
			);
